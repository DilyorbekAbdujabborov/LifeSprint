...
const DEFAULT_STATE = {
  ...
  courses: [
    ...
    { id: "c3", title: "Python va Sun'iy Intellekt Asoslari", description: "Python dasturlash tili asoslari, ma'lumotlar tahlili.", teacherName: "Sardorbek Kobilov", priceCoins: 100, enrolled: false, duration: "3 oy (20 dars)", lessonsCount: 20, rating: 4.7, ratingCount: 88, category: "Dasturlash", color: "amber" },
    { id: "c4", title: "UI/UX Dizayn & Figma Professional", description: "Figma asbobida professional interfeyslar chizishni o'rganing.", teacherName: "Munira Salixova", priceCoins: 90, enrolled: false, duration: "2 oy (12 dars)", lessonsCount: 12, rating: 4.9, ratingCount: 75, category: "Dizayn", color: "rose" }
  ],
  groups: [
    {
      ...
      studentsCount: 0, students: [], rating: 4.8, progress: 0,
      lessons: [
      ...
      homeworks: [
        { id: "hw2-1", title: "Listening Mock Section 1 & 2", deadline: "2026-07-03", status: "pending" },
        { id: "hw2-2", title: "Line Graph Analysis Essay yozish", deadline: "2026-07-08", status: "pending" },
        { id: "hw2-3", title: "Vocabulary List (Topic: Environment) yozib yuklash", deadline: "2026-07-12", status: "pending" }
      ],
      quizzes: [
    ...
    },
    {
      ...
      studentsCount: 0, students: [], rating: 4.9, progress: 0,
      lessons: [
      ...
      homeworks: [
        { id: "hw1-1", title: "ES6+ metodlariga oid 15 ta amaliy masala", deadline: "2026-07-04", status: "pending" },
        { id: "hw1-2", title: "React loyihasi: Ob-havo vidjetini yaratish", deadline: "2026-07-09", status: "pending" }
      ],
      quizzes: [
    ...
    },
    {
      ...
      studentsCount: 0, students: [], rating: 4.7, progress: 0,
      lessons: [
      ...
      homeworks: [
        { id: "hw3-1", title: "Matematik funktsiyalarni klass orqali tuzish", deadline: "2026-07-06", status: "pending" },
        { id: "hw3-2", title: "Web Scraping: Beautiful Soup yordamida ma'lumot olish", deadline: "2026-07-11", status: "pending" }
      ],
      quizzes: [
...
router.get("/", authMiddleware, async (req: AuthedRequest, res: Response) => {
  const row = await db.get("SELECT state FROM user_state WHERE user_id = $1", [req.userId]) as any;
  if (!row) {
  ...
  try {
    ...
    res.json({ state: DEFAULT_STATE });
  }
});

router.put("/", authMiddleware, async (req: AuthedRequest, res: Response) => {
  const state = req.body?.state;
  if (state === undefined) {
    return res.status(400).json({ error: "state maydoni talab qilinadi." });
  }
